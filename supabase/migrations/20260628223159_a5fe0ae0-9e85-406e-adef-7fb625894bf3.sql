CREATE OR REPLACE FUNCTION public.reorder_kanban_card_to_position(_company_id uuid, _card_id uuid, _new_index integer)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE id = _card_id AND company_id = _company_id AND deleted_at IS NULL
  FOR UPDATE;
