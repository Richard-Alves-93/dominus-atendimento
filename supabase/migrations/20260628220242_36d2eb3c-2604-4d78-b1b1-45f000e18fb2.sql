...
BEGIN
  ...
  -- Confirm column belongs to same company
  ...
  -- Get siblings (excluding archived) ordered by position
  ...
  v_filtered := ARRAY(SELECT unnest(v_ids) EXCEPT SELECT _card_id);
  -- Preserve order by re-filtering
  v_filtered := ARRAY(SELECT id FROM unnest(v_ids) AS id WHERE id <> _card_id);

  v_target := GREATEST(0, LEAST(_new_index, array_length(v_filtered, 1) + 1 - 1));
  IF array_length(v_filtered, 1) IS NULL THEN
    v_target := 0;
  END IF;

  -- Insert moving card at v_target
  v_filtered := v_filtered[1:v_target] || ARRAY[_card_id] || v_filtered[v_target+1:array_length(v_filtered,1)];
...
GRANT EXECUTE ON FUNCTION public.reorder_kanban_card_to_position(uuid, uuid, integer) TO authenticated;
