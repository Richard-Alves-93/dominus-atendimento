CREATE OR REPLACE FUNCTION public.pick_next_round_robin_user(
  _company_id uuid,
  _department_id uuid
)
RETURNS TABLE(
...
DECLARE
  ...
  _chosen_name text;
BEGIN
  -- Authorization: caller must belong to the company (or be master)
  IF NOT (
  ...
  -- Lock the department row to serialize concurrent picks
  ...
  -- Pick next eligible user with stable order (created_at, id)
  WITH eligible AS (
  ...
  pick AS (
    ...
    ORDER BY
      CASE
        WHEN lp.rn IS NULL THEN o.rn               -- last not found → first
        ELSE ((o.rn - lp.rn - 1 + o.total) % o.total) -- next after last (wrap)
      END
    LIMIT 1
  )
...
REVOKE ALL ON FUNCTION public.pick_next_round_robin_user(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.pick_next_round_robin_user(uuid, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.pick_next_round_robin_user(uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pick_next_round_robin_user(uuid, uuid) TO service_role;